...
export default function RealtimeDashboard({
  ...
  const getActivityIcon = (type: ActivityItem['type']) => {
    switch (type) {
      case 'petition_created': return '📝';
      case 'petition_approved': return '✅';
  ...
  const getActivityColor = (type: ActivityItem['type']) => {
    switch (type) {
      case 'petition_created': return 'text-blue-600';
      case 'petition_approved': return 'text-green-600';
      case 'signature': return 'text-purple-600';
            ...
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {loading ? <div className="animate-pulse bg-gray-200 h-8 w-12 rounded"></div> : stats.totalPetitions}
                </p>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {loading ? <div className="animate-pulse bg-gray-200 h-8 w-12 rounded"></div> : stats.activePetitions}
                </p>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {loading ? <div className="animate-pulse bg-gray-200 h-8 w-12 rounded"></div> : stats.pendingPetitions}
                </p>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent className="p-6">
            <div className="flex items-center">
              ...
              <div className="ml-4">
                ...
                <p className="text-2xl font-bold text-gray-900">
                  {loading ? <div className="animate-pulse bg-gray-200 h-8 w-12 rounded"></div> : stats.totalSignatures.toLocaleString()}
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-8">
              ...
              <p className="text-gray-600">Create your first petition to see activity here.</p>
            </div>
          ) : (
            <div className="space-y-4">
              ...
              {petitions.slice(0, 5).map((petition) => {
                ...
                return (
                  <div key={petition.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    <div className="flex-1 min-w-0">
                      <h4 className="font-medium text-gray-900 truncate">{petition.title}</h4>
                      <div className="flex items-center gap-4 mt-1">
                        <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium ${
                          petition.status === 'approved' ? 'bg-green-100 text-green-800'
                          : petition.status === 'pending' ? 'bg-yellow-100 text-yellow-800'
                          : petition.status === 'paused' ? 'bg-red-100 text-red-800'
                          : 'bg-gray-100 text-gray-800'
                        }`}>
                          {petition.status}
                        </span>
                        <span className="text-sm text-gray-600">{petition.currentSignatures.toLocaleString()} signatures</span>
